...
class LinkedList {
  ...
  deleteNodeWithoutHead(node) {
    if (node === null || node.next === null)
      console.log("Can not delete the node");
...
list.addLast(1);
list.addLast(9);
const nodeToDelete = list.head.next;
list.deleteNodeWithoutHead(nodeToDelete);
list.printList();
...
class LinkedList {
  ...
  createCycle(index) {
    if (index < 0 || index >= this.size()) console.log("Erorr");
    let targetNode = null;
    let current = this.head;
    for (let i = 0; current !== null; i++, current = current.next) {
      if (i === index) targetNode = current;
    }
    let temp = this.head;
    while (temp.next) {
      temp = temp.next;
...
class LinkedList {
  ...
  splitList() {
    ...
    slow.next = null;
    return { firstHalf: this.head, secondHalf };
  }
...
let { firstHalf, secondHalf } = list.splitList();
...
list.addLast(0);
list.addLast(1);
console.log(list.binaryToDecimal());
